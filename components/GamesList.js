...
export default function GamesList() {
  ...
  games.map(i => i.name === "LittelBoxer" ? i.name = "LittleBoxer" : null); // 修复游戏名称错误
    ...
    <ul className="container mx-auto grid grid-cols-3 gap-1 p-1 scrollbar-hide hover:overflow-y-scroll md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-12">
      { games.map((game) => (
        <li key={ game.id } className="">
          <Image
            src={ `https://cdn.iwantalipstick.com/gameicon2/png/${game.name == "SharkisComing" ? `SharkIsComing` : game.name
              }.png` }
            width={ 100 }
            height={ 100 }
            alt={ game.name }
